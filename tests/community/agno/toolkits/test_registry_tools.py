...
class _PermissionDeniedRegistry(DefaultRegistry):
    """Registry whose searches are refused — exercises the permission surface."""
...
async def test_search_setups_permission_denied_is_distinct() -> None:
    result = json.loads(await RegistryTools(_PermissionDeniedRegistry("", "", "")).search_setups())
    assert result["error"] == "permission denied: search_setups"


async def test_search_modules_permission_denied_is_distinct() -> None:
    result = json.loads(await RegistryTools(_PermissionDeniedRegistry("", "", "")).search_modules())
...
class _EnumDriftRegistry(DefaultRegistry):
    ...
    async def search_setups(self, *args: object, **kwargs: object) -> list:
        msg = "no proto member MODULE_TYPE_X"
        raise ValueError(msg)

    async def search_tools(self, *args: object, **kwargs: object) -> list:
        msg = "no proto member MODULE_TYPE_X"
        raise ValueError(msg)


async def test_enum_drift_is_not_reported_as_retryable() -> None:
    tools = RegistryTools(_EnumDriftRegistry("", "", ""))
    setups = json.loads(await tools.search_setups(kind="tool"))
    modules = json.loads(await tools.search_modules(kind="tool"))
    for result in (setups, modules):
        assert result["error"] == "search filter not supported by this registry version"
        assert "retry" not in result["error"]
