...
class StrictAssertions:
    ...
    @staticmethod
    def assert_mock_called_once_with_exact(
        mock: Mock,
        *args,
        **kwargs
    ):
        ...
        assert mock.call_count == 1, \
            f"Mock {mock} called {mock.call_count} times, expected 1"

        actual_call = mock.call_args
        if actual_call is None:
            raise AssertionError("Mock has no call arguments")
    ...
    @staticmethod
    def assert_async_mock_awaited_once_with_exact(
        mock: Mock,
        *args,
        **kwargs
    ):
        ...
        assert mock.await_count == 1, \
            f"Async mock {mock} awaited {mock.await_count} times, expected 1"

        actual_call = mock.await_args
        if actual_call is None:
            raise AssertionError("Mock has no await arguments")
    ...
    def assert_task_state(
        ...
        expected_cancelled: bool = None,
        expected_exception: Type[Exception] = None
    ):
        """Assert specific state of an asyncio Task."""
        if expected_done is not None:
            assert task.done() == expected_done, \
                f"Task done state is {task.done()}, expected {expected_done}"
    ...
    def assert_dict_subset(
        ...
        for key, expected_value in expected_subset.items():
            assert key in actual, \
                f"Key '{key}' not found in actual dict. Keys: {list(actual.keys())}"
            ...
            if strict_types:
                assert type(actual_value) == type(expected_value), \
                    ...
                    f"{type(actual_value)} != {type(expected_value)}"
    ...
    @staticmethod
    def assert_no_exceptions_logged(caplog):
        """Assert no exceptions were logged."""
        for record in caplog.records:
            assert not record.exc_info, \
                f"Exception logged at {record.pathname}:{record.lineno}: " \
                f"{record.getMessage()}"
            ...
            for keyword in exception_keywords:
                if keyword in msg_lower and record.levelname == 'ERROR':
                    raise AssertionError(
                        f"Potential exception in log at level {record.levelname}: "
                        f"{record.getMessage()}"
                    )

    @staticmethod
    async def assert_async_cleanup(
        setup_coro: Callable,
        cleanup_coro: Callable,
        check_coro: Callable
    ):
        ...
        try:
            ...
            assert await check_coro(resources), \
                "Resources not properly created during setup"

            # Run cleanup
            await cleanup_coro(resources)

            # Verify resources are cleaned
            assert not await check_coro(resources), \
                "Resources not properly cleaned during cleanup"
        except Exception:
    ...
    def assert_module_implementation(
        ...
        for method_name in required_methods:
            ...
            assert callable(method), \
                f"{method_name} is not callable in {module_class.__name__}"
        ...
        class ResourceLimitChecker:
            ...
            def register_task(self):
                ...
                if max_tasks is not None:
                    assert self.task_count <= max_tasks, \
                        f"Task limit exceeded: {self.task_count} > {max_tasks}"
            ...
            def check_memory(self):
                ...
                assert peak_mb <= max_memory_mb, \
                    f"Memory limit exceeded: {peak_mb:.2f} MB > {max_memory_mb} MB"

            def assert_all_released(self):
                """Assert all resources have been released."""
                assert self.task_count == 0, \
                    f"Tasks not released: {self.task_count} still active"
                assert self.connection_count == 0, \
                    f"Connections not released: {self.connection_count} still active"
...
def strict_task_validator():
    ...
    class TaskValidator:
        ...
        async def assert_all_complete(self, timeout: float = 1.0):
            ...
            try:
                ...
                raise AssertionError(
                    f"{len(incomplete)} tasks did not complete within {timeout}s"
                )
        ...
        def assert_call_order(self, *expected_calls: str):
            ...
            for i, expected_name in enumerate(expected_calls):
                ...
                assert actual_name == expected_name, \
                    f"Call order mismatch at position {i}: " \
                    f"expected {expected_name}, got {actual_name}"

        def assert_no_unexpected_calls(self):
...
def strict_queue_validator():
    ...
    class QueueValidator:
        ...
        async def assert_queue_contents(self, name: str):
        ...
        def assert_all_empty(self):
            """Assert all registered queues are empty."""
            for name, queue in self.queues.items():
                assert queue.empty(), \
                    f"Queue '{name}' is not empty: {queue.qsize()} items remaining"

        def assert_all_bounded(self):
...
def strict_exception_handler():
    ...
    class ExceptionHandler:
        ...
        def assert_no_unhandled_exceptions(self):
            """Assert no unhandled exceptions occurred."""
            assert not self.exceptions, \
                f"Unhandled exceptions detected: {self.exceptions}"

        def assert_exception_type(self, exc_type: Type[Exception]):
            """Assert specific exception type was caught."""
            for exc in self.exceptions:
                if isinstance(exc, exc_type):
                    return
            raise AssertionError(
                f"Expected exception of type {exc_type}, " \
                f"got: {[type(e) for e in self.exceptions]}"
            )
