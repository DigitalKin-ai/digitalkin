...
class TestTtlProductionWorkflows:
    ...
    async def test_task_state_ttl_lifecycle(self) -> None:
        """Task state hash created with 24h TTL, queried, deleted."""
        client = fakeredis_aio.FakeRedis()

        pipe = client.pipeline()
        pipe.hset("task:t1", mapping={"status": "running", "started_at": "2025-01-01"})
        pipe.expire("task:t1", 86400)
        await pipe.execute()

        assert await client.ttl("task:t1") > 86000

        await client.delete("task:t1")
        assert await client.ttl("task:t1") == -2
