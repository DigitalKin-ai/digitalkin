...
class TestStreamWriterMemory:
    """ProtoStreamWriter/Reader lifecycle memory stability."""

    async def test_writer_reader_cycle_no_growth(self) -> None:
        """100 writer/reader create-destroy cycles: stable memory."""
        ...
        for i in range(100):
            writer = ProtoStreamWriter(f"mem:sw:{i}", client)
            s = struct_pb2.Struct()
            s.update({"cycle": i})
            await writer.write_struct(s)
            await writer.write_eos()

            reader = ProtoStreamReader(f"mem:sw:{i}", client)
        ...
        rss_delta = rss_after - rss_before
        assert rss_delta < 30, f"RSS grew by {rss_delta:.1f}MB over 100 writer/reader cycles"
