...
class TestModuleServicerInvalidation:
    ...
    def test_invalidate_setup_cache_clears_both_dicts(self) -> None:
        ...
        servicer._setup_inflight = {"s1": asyncio.Future()}
...
class TestGatewayServicerCacheSignals:
    ...
    @pytest.mark.asyncio
    async def test_invalidate_all_calls_handler(self, gateway) -> None:
        """INVALIDATE_ALL dispatches to cache_handler."""
    ...
    @pytest.mark.asyncio
    async def test_invalidate_setup_calls_handler(self, gateway) -> None:
        """INVALIDATE_SETUP dispatches to cache_handler."""
        from agentic_mesh_protocol.gateway.v1 import gateway_pb2
        ...
        assert resp.success is True
        gateway._cache_handler.assert_awaited_once_with("INVALIDATE_SETUP")

    @pytest.mark.asyncio
    async def test_invalidate_shared_calls_handler(self, gateway) -> None:
        """INVALIDATE_SHARED dispatches to cache_handler."""
        from agentic_mesh_protocol.gateway.v1 import gateway_pb2
    ...
    async def test_invalidate_without_handler_returns_false(self) -> None:
        """INVALIDATE_* without cache_handler returns success=False."""
        from agentic_mesh_protocol.gateway.v1 import gateway_pb2
        from digitalkin.grpc_servers.gateway_servicer import GatewayServicer

        gw = GatewayServicer(redis_client=MagicMock(), cache_handler=None)
        request = gateway_pb2.ClientSignalRequest(
            action=gateway_pb2.SignalAction.Value("INVALIDATE_ALL"),
        )
...
class TestModuleServerCacheHandlers:
    ...
    @pytest.mark.asyncio
    async def test_invalidate_all_calls_all_handlers(self) -> None:
        """INVALIDATE_ALL calls all individual invalidation methods."""
        from digitalkin.grpc_servers.module_server import ModuleServer

        server = MagicMock(spec=ModuleServer)
        server._invalidate_setup = AsyncMock()
        server._invalidate_tools = AsyncMock()
        ...
        server._invalidate_setup.assert_awaited_once()
        server._invalidate_tools.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    async def test_invalidate_setup_calls_servicer(self) -> None:
        """INVALIDATE_SETUP calls module_servicer.invalidate_setup_cache."""
        from digitalkin.grpc_servers.module_server import ModuleServer
