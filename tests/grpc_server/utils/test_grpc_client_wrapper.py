...
class _FakeStub:
    """Stand-in stub class: ``stub_class(channel)`` constructs on all Python versions.

    Avoids ``MagicMock(channel)``, which Python 3.12 rejects (the channel mock would
    be treated as a spec → ``InvalidSpecError: Cannot spec a Mock object``).
    """
...
class TestChannelCache:
    ...
    def test_channel_wires_permission_interceptor(self, mock_insecure_channel: MagicMock) -> None:
        """Every channel carries the permission middleware so no call can bypass it."""
        mock_insecure_channel.return_value = MagicMock()

        GrpcClientWrapper()._init_channel(_make_config())

        interceptors = mock_insecure_channel.call_args.kwargs["interceptors"]
        assert any(isinstance(i, PermissionClientInterceptor) for i in interceptors)
...
class TestStubCache:
    ...
    async def test_stub_cache_evicted_on_channel_close(self, mock_insecure_channel: MagicMock) -> None:
        ...
        wrapper = GrpcClientWrapper()
        wrapper._init_channel(_make_config())
        wrapper._get_or_create_stub(_FakeStub)
    ...
    def test_no_cache_key_returns_fresh_stub(self, mock_insecure_channel: MagicMock) -> None:
        ...
        stub = wrapper._get_or_create_stub(_FakeStub)
