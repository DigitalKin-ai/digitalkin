...
def _mock_servicer(redis_client: Any = "default_mock", **kwargs: Any) -> Any:
    ...
    return GatewayServicer(redis_client=redis_client, **kwargs)
...
class TestStreamLateConsumer:
    ...
    async def test_reads_from_redis_when_session_gone(self, redis: Any) -> None:
        ...
        # Simulate module output already written to Redis (production xadd format)
        from google.protobuf import struct_pb2

        stream_key = f"task:{task_id}:stream"
        s = struct_pb2.Struct()
        s.update({"root": {"protocol": "message", "text": "hello"}})
        await redis.xadd(stream_key, {"pb": s.SerializeToString(), "seq": "1"})
