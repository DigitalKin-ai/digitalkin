...
# ===========================================================================
# start() — boot-time Redis pool pre-warm
# ===========================================================================


class TestGatewayStart:
    """Boot-time pre-warm pings both Redis pools and fails fast if Redis is down."""

    async def test_calls_verify_at_boot(self) -> None:
        """``start()`` pings Redis (warming both pools) before m2m starts."""
        redis_client = MagicMock()
        redis_client.verify = AsyncMock(return_value=True)
        redis_client.url = "redis://localhost:6379/0"
        servicer = _mock_servicer(redis_client=redis_client)
        servicer._m2m.start = AsyncMock()  # noqa: SLF001

        await servicer.start()

        redis_client.verify.assert_awaited_once()
        servicer._m2m.start.assert_awaited_once()  # noqa: SLF001

    async def test_raises_when_redis_unreachable(self) -> None:
        """``start()`` raises ``RedisUnreachableError`` if verify() returns False — crash-loud."""
        from digitalkin.core.exceptions import RedisUnreachableError

        redis_client = MagicMock()
        redis_client.verify = AsyncMock(return_value=False)
        redis_client.url = "redis://localhost:6379/0"
        servicer = _mock_servicer(redis_client=redis_client)
        servicer._m2m.start = AsyncMock()  # noqa: SLF001
        ...
        servicer._m2m.start.assert_not_awaited()  # noqa: SLF001
...
class TestStartStream:
    ...
    async def test_capacity_exceeded_returns_not_accepted(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        for i in range(get_gateway_settings().max_streams):
