"""storage_id stamping, record-scoped listing and collection removal on the local strategy."""
...
class TestListPagination:
    ...
    async def test_record_id_does_not_match_on_prefix(self, storage: DefaultStorage) -> None:
        """Record "r1" must not drag in "r10" — the key sweep is prefix-based underneath."""
    ...
    async def test_unspecified_visibility_constrains_nothing(self, storage: DefaultStorage) -> None:
        """UNSPECIFIED is dropped from the filter, as the storage service requires."""
        await storage.store("c", "pub", {"v": 0}, visibility=Visibility.PUBLIC)
        await storage.store("c", "priv", {"v": 1}, visibility=Visibility.PRIVATE)

        assert [r.record_id for r in await storage.list("c", visibilities=[Visibility.UNSPECIFIED])] == ["pub", "priv"]
        found = await storage.list("c", visibilities=[Visibility.UNSPECIFIED, Visibility.PRIVATE])
        assert [r.record_id for r in found] == ["priv"]


class TestCollectionRemoval:
    """remove_collection() wipes the whole collection; single records go through remove()."""

    async def test_wipes_every_record_of_the_collection(self, storage: DefaultStorage) -> None:
        for i in range(3):
            await storage.store("c", f"r{i}", {"v": i})

        assert await storage.remove_collection("c") is True
        assert await storage.list("c") == []
        assert not any(k.startswith("missions:m1|c:") for k in storage._record_locks)

    async def test_record_removal_does_not_evict_sibling_locks(self, storage: DefaultStorage) -> None:
        ...
        assert await storage.remove("c", "r1") is True

        assert "missions:m1|c:r1" not in storage._record_locks
        assert sibling_key in storage._record_locks
        assert [r.record_id for r in await storage.list("c")] == ["r10"]
