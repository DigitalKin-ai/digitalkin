...
from digitalkin.models.services.filesystem import FileType, UploadFileData, FileStatus, FilesystemRecord, FileFilter
...
class TestDefaultFilesystem:
    ...
    async def test_upload_files_success(
        ...
        # Upload the file
        files, total_uploaded, total_failed = await filesystem.upload([upload_file])
        assert len(files) == 1
    ...
    async def test_get_file_success(
        ...
        files, _, _ = await filesystem.upload([upload_file])
        file_id = files[0].id

        # Get the file
        file_data = await filesystem.get(file_id)
        assert isinstance(file_data, FilesystemRecord)
    ...
    async def test_get_files_success(
        ...
        _files, _, _ = await filesystem.upload(upload_files)

        # Create filter criteria
        filters = FileFilter(types=[file_metadata["type"]])

        # Get the files
        result_files, total_count = await filesystem.list(filters, include_content=False)
    ...
    async def test_update_file_success(
        ...
        updated_file = await filesystem.update(
            file_id,
    ...
    async def test_delete_files_success(
        ...
        files, _, _ = await filesystem.upload(upload_files)
        ...
        # Delete the files
        results, total_deleted, total_failed = await filesystem.delete(
            filters,
            permanent=True,
            force=False,
        )
    ...
    async def test_get_file_nonexistent(self, filesystem: DefaultFilesystem) -> None:
        ...
        with pytest.raises(FilesystemServiceError):
            await filesystem.get("nonexistent_file_id")
    ...
    async def test_upload_files_duplicate_error(
        ...
        await filesystem.upload([upload_file])
    ...
    async def test_upload_files_replace_existing(
        ...
        files, total_uploaded, total_failed = await filesystem.upload([upload_file_replace])
        assert len(files) == 1
        assert total_uploaded == 1
        assert total_failed == 0
    ...
    async def test_get_files_with_filters(
        ...
        files, _, _ = await filesystem.upload(files_to_upload)

        # Update one file to ARCHIVED status
        await filesystem.update(files[1].id, status=FileStatus.ARCHIVED)

        # Test filtering by type
        filters = FileFilter(types=[FileType.DOCUMENT])
        result_files, total_count = await filesystem.get(filters)
        ...
        result_files, total_count = await filesystem.list(filters)
    ...
    async def test_get_files_pagination(
        ...
        await filesystem.upload(files_to_upload)

        # Test pagination with list_size=2
        filters = FileFilter()

        # First page
        result_files, total_count = await filesystem.list(filters, pagination=PaginationRequest(limit=2, offset=0))
        assert len(result_files) == 2
        assert total_count == 5

        # Second page
        result_files, total_count = await filesystem.list(filters, pagination=PaginationRequest(limit=2, offset=2))
        assert len(result_files) == 2
        assert total_count == 5

        # Last page
        result_files, total_count = await filesystem.list(filters, pagination=PaginationRequest(limit=2, offset=4))
        assert len(result_files) == 1
        assert total_count == 5
    ...
    async def test_delete_files_soft_delete(
        ...
        results, total_deleted, total_failed = await filesystem.delete(filters, permanent=False)
