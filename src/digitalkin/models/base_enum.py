"""Base enum with protobuf conversion support."""
...
class BaseEnum(Generic[P]):
    ...
    def _get_proto_enum(cls) -> type[P] | None:
        ...
        Raises:
            AttributeError: If proto enum type not found in generic parameters.
        """
    ...
    def to_proto(self) -> P | None:
        """Convert this enum value to its protobuf equivalent.

        Returns:
            The protobuf enum value, or None if conversion fails.
        """
        try:
            proto_enum = self.__class__._get_proto_enum()
            if proto_enum is None:
                return None
            if "UNSPECIFIED" in self.value:
                return proto_enum.Value(proto_enum.keys()[0])  # retourne 0
