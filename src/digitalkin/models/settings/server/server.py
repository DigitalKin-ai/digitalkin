...
class ServerSettings(BaseSettings):
    ...
    asyncio_inspector_state: bool = Field(default=False, description="Enable asyncio inspector")
    asyncio_inspector_port: int = Field(default=8765, description="Port for asyncio inspector")
    setup_cache_max: NonNegativeInt = Field(
        default=100, description="Maximum number of cached setups in ModuleServicer"
    )
    completion_timeout: float = Field(default=300.0, description="Timeout for completion in ModuleServicer")
