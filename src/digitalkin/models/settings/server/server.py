...
class ServerSettings(BaseSettings):
    ...
    max_concurrent_rpcs: NonNegativeInt = Field(
        os.cpu_count() * 200, description="Maximum number of RPCs handled in parallel by the server."
    )
    ...
    thread_pool_workers: NonNegativeInt = Field(
        min(4, os.cpu_count()), description="Number of workers in the server thread pool."
    )
