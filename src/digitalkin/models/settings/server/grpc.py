...
class GrpcServerSettings(BaseSettings):
    ...
    Attributes:
        ...
        max_pings_without_data (NonNegativeFloat): Maximum number of pings the server allows without receiving any data. Setting to 0 allows unlimited pings, which is important for long-running streams.
        keepalive_permit_without_calls (bool): Allow clients to send keepalive pings even when there are no active RPCs. This is important for keeping connections alive through proxies and detecting dead clients.
    ...
    max_pings_without_data: NonNegativeFloat = Field(
        0,
        description="Maximum number of pings the server allows without receiving any data. Setting to 0 allows unlimited pings, "
    ...
    keepalive_permit_without_calls: bool = Field(
        True,
        description="Allow clients to send keepalive pings even when there are no active RPCs. This is important for keeping connections "
