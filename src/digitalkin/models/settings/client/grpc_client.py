...
class ClientGrpcSettings(BaseGrpcSettings):
    ...
    dns_resolution_time: NonNegativeInt = Field(
        default=500, description="Minimum time between DNS resolutions in milliseconds"
    )
    initial_reconnect_time: NonNegativeInt = Field(
        default=1000, description="Initial reconnect backoff time in milliseconds"
    )
    max_reconnect_time: NonNegativeInt = Field(
        default=10000, description="Maximum reconnect backoff time in milliseconds"
    )
    min_reconnect_time: NonNegativeInt = Field(
        default=500, description="Minimum reconnect backoff time in milliseconds"
    )
    min_ping_interval_time: NonNegativeInt = Field(
        default=30000, description="Minimum time between HTTP/2 pings in milliseconds"
    )
    enable_retries: bool = Field(default=True, description="Enable gRPC retries")
