...
class TaskSession:
    ...
    started_at: object  # datetime or None
    completed_at: object  # datetime or None
    ...
    def __init__(
        ...
        Args:
            ...
            module: Module instance (signal service derived from module.context.task_manager)
    ...
    async def listen_signals(self) -> None:
        ...
        Subscribes to the signal service and processes incoming signals.
        ...
        sub_id, signal_gen = await self.signal_service.subscribe_signals(self.task_id)
        try:
            async for signal in signal_gen:
                ...
                if signal.get("task_id") != self.task_id:
                    continue

                action = signal.get("action")
                if action == SignalType.CANCEL.value:
