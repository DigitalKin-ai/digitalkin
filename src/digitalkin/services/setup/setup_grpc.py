...
class GrpcSetup(SetupStrategy, GrpcClientWrapper, GrpcErrorHandlerMixin):
    ...
    def create(self, setup_dict: dict[str, Any]) -> str:
        with self.handle_grpc_errors("CreateSetup", SetupServiceError):
    ...
    def get(self, setup_dict: dict[str, Any]) -> SetupData:
        with self.handle_grpc_errors("GetSetup", SetupServiceError):
    ...
    def update(self, setup_dict: dict[str, Any]) -> bool:
        ...
        with self.handle_grpc_errors("SetupUpdate", SetupServiceError):
            valid_data = SetupData.model_validate(setup_dict)
    ...
    def delete(self, setup_dict: dict[str, Any]) -> bool:
        with self.handle_grpc_errors("SetupDeletion", SetupServiceError):
    ...
    def list(self, list_dict: dict[str, Any]) -> dict[str, Any]:
        with self.handle_grpc_errors("ListSetups", SetupServiceError):
