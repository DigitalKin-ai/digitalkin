    ...
    def _filter_to_proto(self, filters: FileFilter) -> filesystem_messages_pb2.FileFilter:
        ...
        return filesystem_messages_pb2.FileFilter(
            **filters.model_dump(exclude={"file_types", "status", "context"}),
            types=[FileType.from_proto(file_type) for file_type in filters.types] if filters.types else None,
            status=FileStatus.from_proto(filters.status) if filters.status else None,
            context=context_id,
        )
