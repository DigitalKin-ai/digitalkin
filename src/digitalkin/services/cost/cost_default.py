...
class DefaultCost(CostStrategy):
    ...
    async def create(
        ...
        """Create a cost record in local storage.

        Raises:
            CostServiceError: If cost config not found or duplicate name.
        """
    ...
    async def set_config(self, configs: list[CostConfig]) -> bool:
        """Store cost configs in memory.

        Returns:
            True when configs are stored.
        """
    ...
    async def list(
        ...
        """List cost records filtered by names or types.

        Returns:
            Filtered list of cost data.

        Raises:
            CostServiceError: If mission not found.
        """
