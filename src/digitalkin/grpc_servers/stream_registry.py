    ...
    def _on_monitored_task_done(self, task: asyncio.Task[Any]) -> None:
        ...
        if name.startswith("dial_consumer_"):
            ...
            if task_id in self._local_cache:
                ...
                reap_task = loop.create_task(self._reap_local(task_id), name=f"reap_{task_id}")
                self._monitored_tasks.add(reap_task)
                reap_task.add_done_callback(self._on_monitored_task_done)
