...
class GrpcClientWrapper:
    ...
    _channel_pool: dict[str, grpc.aio.Channel] | None = None
    ...
    def _init_channel(self, config: ClientConfig) -> grpc.aio.Channel:
        """Create an async gRPC channel with instance-level pooling.
        ...
        cache_key = config.address
        if cache_key in self._channel_pool:
            return self._channel_pool[cache_key]
        ...
        self._channel_pool[cache_key] = channel
        return channel
    ...
    async def exec_grpc_query(
        ...
        Arguments:
            ...
            timeout: Optional timeout in seconds. None means no deadline.
