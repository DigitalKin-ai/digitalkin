...
class CircuitBreaker:
    ...
    @classmethod
    def get_or_create(cls, service_id: str) -> CircuitBreaker:
    ...
    def release_probe(self) -> bool:
        """Release a half-open probe slot without recording an outcome.

        Called when an in-flight probe is abandoned (e.g. the caller was
        cancelled) so the breaker cannot wedge with the lock held. A cancelled
        probe is neither success nor failure — the slot simply frees for the
        next caller. No-op unless a probe lock is currently held.

        Returns:
            True if a held probe lock was released; False if none was held.
        """
        if self._state == CBState.HALF_OPEN and self._half_open_lock:
            self._half_open_lock = False
            return True
        return False
